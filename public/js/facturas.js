	...
	var oTable = $("#tablaDatos").dataTable({
        ...
        serverSide: true,
        ajax: {
            url: '/admin/facturas/getfacturas',
            data: function (d) {
        ...
        columns: [
            ...
            { data: 'total_fact', name: 'total_fact',  orderable: false, searchable: false},
    ...
    $(document).on('click', '#generate', function(btn) {
	    
	    //alert($("#costo").find('option:selected').remove());
        ...
        $.ajax({
	        ...
	        url: "/admin/facturas/generate",
	        dataType: 'json',
	        data: {
	        	'fecha': $("#costo option:selected").text(),
	    	},
	        
	        success: function(data) {
	            if (data == 'listo') {
	            	$("#costo").find('option:selected').remove();
	            	oTable.fnDraw();
	            }
	        }
	    });
